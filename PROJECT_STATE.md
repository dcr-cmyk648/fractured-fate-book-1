...
- Apps Script private deployment configuration must be preserved locally but not synced to GitHub. Keep real Drive folder IDs, submitted-comments folder IDs, Sheet IDs, reader codes, account lists, import tokens, and deployment settings out of repository files. When updating `feedback/webapp/import-webapp-comments.gs`, preserve the deployed Apps Script project's existing private `CONFIG` values unless the author explicitly wants to rotate or replace them.
