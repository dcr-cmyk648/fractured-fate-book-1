---
id: magic-shroud
entity_type: magic
display_name: Shroud / Shrouds
status: candidate
last_reviewed: 2026-06-24
review_packet: reviews/entities/magic/magic-shroud.md
accepted_file: bible/magic/magic-shroud.md
---

# Shroud Candidate Material

The accepted Shroud record stores the throughput/anchoring/maintenance model. This file preserves unresolved or later-review material without treating it as accepted canon.

## Ink Interaction

- Exact Ink-before-Shroud timing remains tentative. Current author direction is that Ink may need to be received before Shroud manifestation.
- Whether Ink must be received before Shroud manifestation as a hard rule, or only for certain effects, remains unresolved.
- Whether Ink-adjacent abilities temporarily interrupt or destabilize a Shroud remains unresolved.

Route to `magic-ink-boons`.

## Selwin

- Selwin currently cannot form a Shroud.
- It remains unresolved whether he can ever form a normal Shroud, form a nonstandard equivalent, or remains incompatible because of heritage or materially different abilities.

Route to Selwin, Alara, Naeroth, Dissolution, and later series-endgame reviews.

## Overlap and Mental Contest

Planning suggests Shroud-vs-Shroud or magic-vs-magic can become a mental struggle over who gets to bend a portion of reality to their will.

This should be reviewed during magic-combat mechanics, Shroud overlap scenes, and later Vorren/Serathis/Maya/Davian combat planning.

## Body Training Interface

Archeon/Naeroth-adjacent body training may help internalize instincts needed for Shroud manifestation and combat use.

Exact mechanics remain for Archeon, Naeroth, martial training, and magic-system reviews.

## Older Magic-Origin Notes

Accepted high-level boundary: the original sin softened reality and allowed modern Clade magic to become extremely overpowered and unbalanced. Older or primeval magic being more balanced is true at the high level.

Older source-file implications about exact Shroud architecture, original-magic taxonomy, or direct two-system interfaces are dated and should not override the current Shroud, Kinesis, Ink, and Convergence owner files unless separately reopened.

## Kinesis Interaction

Exact kinesis/Shroud interaction remains candidate.

Possibilities to preserve:

- kinesis may interact with Shroud sublimation and backlash handling
- kinesis may be difficult to combine with Shroud maintenance because both require energy handling and attention
- Shrouds may protect the user from kinesis backlash without making the systems fully integrated

## Provenance

- `imports/normalized/book-1-outline.md:360-390`
- `imports/normalized/book-1-outline.md:7185-7235`
- `imports/normalized/book-1-outline.md:7305-7330`
- `imports/normalized/bullet-notes.md:183`
- `imports/normalized/bullet-notes.md:267`
- `reviews/entities/magic/magic-shroud.md`
